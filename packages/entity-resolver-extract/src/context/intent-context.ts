...
const BUILTIN_INTENTS: IntentDefinition[] = [
  {
    ...
    fields: [
      ...
      },
      {
        ...
        boost: 0.05,
        synonyms: ['for', '标题', '名前'],
        description: 'Alarm label/title',
      },
      {
        name: 'repeat',
        type: 'string',
        boost: 0.03,
        synonyms: ['repeat', '重复', 'every'],
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        boost: 0.05,
        synonyms: ['in', 'from', '来源'],
        description: 'Search source/platform',
      },
      {
        name: 'limit',
        type: 'number',
        boost: 0.02,
        synonyms: ['top', 'count', '数量'],
 ...
 */
export function resolveIntent(intentHint?: string, text?: string): IntentContextResult | undefined {
