...
export function renderWaterfallSvg(data: WaterfallChartData, containerWidth: number = 600): SVGSVGElement {
  const d = chartDims(containerWidth, 3);
  const svg = svgEl('svg', { width: d.width, height: d.height, viewBox: `0 0 ${d.width} ${d.height}`, role: 'img', 'aria-label': `Waterfall chart: match weight ${data.totalWeight.toFixed(2)}` });
  ...
  // Zero line
  g.appendChild(svgEl('line', { x1: 0, y1: zeroY, x2: d.innerW, y2: zeroY, stroke: '#9aa0a6', 'stroke-dasharray': '4,4', 'stroke-width': '1' }));

  // Y-axis labels
  for (let v = Math.ceil(-absMax); v <= Math.ceil(absMax); v += Math.max(1, Math.ceil(absMax / 4))) {
    const yPos = zeroY - (v / absMax) * d.innerH * 0.9;
    if (yPos >= 0 && yPos <= d.innerH) {
      g.appendChild(svgEl('line', { x1: 0, y1: yPos, x2: d.innerW, y2: yPos, stroke: '#eee', 'stroke-width': '0.5' }));
      const txt = svgEl('text', { x: -5, y: yPos + 4, 'text-anchor': 'end', 'font-size': '10', fill: '#5f6368' });
  ...
  for (let i = 0; i < data.bars.length; i++) {
    ...
    // Connector line
    g.appendChild(svgEl('line', { x1: 10, y1: barY + barH2 / 2, x2: 30, y2: barY + barH2 / 2, stroke: '#dadce0', 'stroke-width': '1' }));

    // Bar
    g.appendChild(svgEl('rect', { x: 30, y: barY, width: d.innerW - 120, height: Math.max(1, barH2), fill: color, rx: '3' }));

    // Label
    const lbl = svgEl('text', { x: 0, y: barY + barH2 / 2 + 4, 'font-size': '11', fill: '#202124', 'font-family': 'system-ui' });
    lbl.textContent = bar.label;
    g.appendChild(lbl);

    // Value
    const val = svgEl('text', { x: d.innerW - 5, y: barY + barH2 / 2 + 4, 'text-anchor': 'end', 'font-size': '10', fill: '#5f6368' });
  ...
  // Title
  const title = svgEl('text', { x: d.width / 2, y: 16, 'text-anchor': 'middle', 'font-size': '13', 'font-weight': 'bold', fill: '#202124', 'font-family': 'system-ui' });
...
export function renderHistogramSvg(data: HistogramData, containerWidth: number = 600): SVGSVGElement {
  const d = chartDims(containerWidth, 2.5);
  const svg = svgEl('svg', { width: d.width, height: d.height, viewBox: `0 0 ${d.width} ${d.height}`, role: 'img' });
  svg.appendChild(svgEl('rect', { x: 0, y: 0, width: d.width, height: d.height, fill: '#fff' }));
  const g = svgEl('g', { transform: `translate(${d.margin.left},${d.margin.top})` });
  ...
  for (let v = 0; v <= maxCount; v += Math.max(1, Math.ceil(maxCount / 4))) {
    ...
    g.appendChild(svgEl('line', { x1: 0, y1: y, x2: d.innerW, y2: y, stroke: '#eee', 'stroke-width': '0.5' }));
    const txt = svgEl('text', { x: -5, y: y + 4, 'text-anchor': 'end', 'font-size': '10', fill: '#5f6368' });
  ...
  if (data.threshold !== undefined) {
    const threshX = 20 + (Math.max(0, data.threshold - (data.bins[0]?.minWeight ?? 0)) / ((data.bins[data.bins.length - 1]?.maxWeight ?? 1) - (data.bins[0]?.minWeight ?? 0))) * (d.innerW - 40);
    g.appendChild(svgEl('line', { x1: threshX, y1: 0, x2: threshX, y2: d.innerH, stroke: '#ea4335', 'stroke-width': '1.5', 'stroke-dasharray': '6,3' }));
  ...
  for (let i = 0; i < data.bins.length; i++) {
    ...
    const y = d.innerH - h;
    const fill = data.threshold !== undefined && bin.minWeight >= data.threshold ? '#34a853' : '#1a73e8';
    ...
    if (i % Math.max(1, Math.floor(data.bins.length / 8)) === 0) {
      const xlbl = svgEl('text', { x: x + binW / 2, y: d.innerH + 16, 'text-anchor': 'middle', 'font-size': '9', fill: '#5f6368' });
...
export function renderEvaluationRadarSvg(data: EvaluationRadarData, containerWidth: number = 400): SVGSVGElement {
  ...
  for (let level = 1; level <= 4; level++) {
    svg.appendChild(svgEl('circle', { cx, cy, r: r * level / 4, fill: 'none', stroke: '#eee', 'stroke-width': '0.5' }));
    const lbl = svgEl('text', { x: cx, y: cy - r * level / 4 - 3, 'text-anchor': 'middle', 'font-size': '8', fill: '#dadce0' });
    lbl.textContent = String((level / 4).toFixed(2));
    svg.appendChild(lbl);
  }
  ...
  for (let i = 0; i < N; i++) {
    ...
    svg.appendChild(svgEl('line', {
      x1: cx, y1: cy, x2: cx + r * Math.cos(angle), y2: cy + r * Math.sin(angle),
      stroke: '#eee', 'stroke-width': '0.5',
    }));
    const lbl = svgEl('text', {
      x: cx + (r + 14) * Math.cos(angle), y: cy + (r + 14) * Math.sin(angle) + 4,
      'text-anchor': 'middle', 'font-size': '9', fill: '#202124',
    });
  ...
  svg.appendChild(svgEl('polygon', { points, fill: 'rgba(26,115,232,0.2)', stroke: '#1a73e8', 'stroke-width': '2' }));

  // Center label
  const f1 = data.axes.find(a => a.name === 'Pairwise F1')?.value ?? 0;
  const centerLabel = svgEl('text', { x: cx, y: cy + 4, 'text-anchor': 'middle', 'font-size': '10', 'font-weight': 'bold', fill: '#202124' });
