...
import { autoConfigure, connectedComponents, evaluateClustering, estimateParameters, computeAggregateMatchWeight } from '@agentix-e/entity-resolver-core';
...
describe('Real DBLP-ACM — DuckDB SQL E2E', () => {
  it('achieves F1 > 0.92 on real DBLP-ACM with SQL-only pipeline', async () => {
    ...
    const comparisons = [
      { field: 'title', scorerName: 'exact', levels: [
          { label: 'exact_match', threshold: 0.99 },
          { label: 'not_match', threshold: 0.0 },
      ]},
      { field: 'authors', scorerName: 'tokenSort', levels: [
          { label: 'exact_match', threshold: 0.99 },
    ...
    const f1 = em.clusterPrecision + em.clusterRecall > 0
