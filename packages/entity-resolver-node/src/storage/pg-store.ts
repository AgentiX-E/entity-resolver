...
export class PgEntityStore {
  ...
  async getEntity(id: EntityId): Promise<{ clusterId: string; memberIds: number[]; cohesion: number } | null> {
    ...
    const row = result.rows[0] as { cluster_id: string; member_ids: number[]; cohesion: number };
    return { clusterId: row.cluster_id, memberIds: row.member_ids, cohesion: row.cohesion };
  }

  /** Query neighboring entities by member overlap. */
  async queryNeighbors(id: EntityId, hops?: number): Promise<{ clusterId: string; memberIds: number[]; cohesion: number }[]> {
  ...
  async applyMerge(from: EntityId, into: EntityId): Promise<void> {
    ...
    try {
      ...
      const fromResult = await client.query(
        'SELECT member_ids FROM er_entities WHERE cluster_id = $1', [from],
      );
      if (fromResult.rows.length > 0) {
        const fromMembers = (fromResult.rows[0] as any).member_ids as number[];
        await client.query(
