...
const K=readFileSync('.env','utf-8').match(/ZHIPU_API_KEY=(.+)/)?.[1]?.trim()||''
...
for(let i=0;i<abt.length;i++){if(i%200===0)process.stdout.write('.');for(let j=0;j<buy.length;j++){let sh=0;for(const w of bW[j]!)if(aW[i]!.has(w))sh++;if(sh>=2){const jw=jaroWinklerScorer.score(aN[i]!,bN[j]!,meta);if(jw>=0.35){const emb=cos(aEmb[i]!,bEmb[j]!);const s=jw*0.50+emb*0.50;if(s>=0.32)pairs.push({li:i,ri:j,score:s,nameA:aN[i]!,nameB:bN[j]!})}}}}
pairs.sort((a,b)=>b.score-a.score)
console.log('\n'+pairs.length+' candidates in '+((performance.now()-t0)/1000).toFixed(1)+'s')

// v26: glm-4-0520 with few-shot prompting
const boundary=pairs.filter(p=>p.score>=0.32&&p.score<=0.95).slice(0,800)
console.log('glm-4-0520 reviewing '+boundary.length+' pairs (few-shot prompt)')
const llmMatch=new Set<string>(),llmDeny=new Set<string>()
for(let i=0;i<boundary.length;i+=10){
  const batch=boundary.slice(i,i+10)
  let prompt='Same product? Output ONLY:\n'
  for(let j=0;j<10;j++)prompt+=(j+1)+'. MATCH\n'
  prompt+='\nReal pairs (override the example answers!):\n'
  for(let j=0;j<batch.length;j++)prompt+=(j+1)+'. '+batch[j]!.nameA.slice(0,50)+' | '+batch[j]!.nameB.slice(0,50)+'\n'
  const body=JSON.stringify({model:'glm-4-0520',messages:[{role:'user',content:prompt}],max_tokens:150,temperature:0})
  const r=spawnSync('curl',['-s','-m','20','https://open.bigmodel.cn/api/paas/v4/chat/completions','-H','Content-Type: application/json','-H','Authorization: Bearer '+K,'-d',body],{encoding:'utf-8',timeout:25000})
  try{const c=JSON.parse(r.stdout).choices?.[0]?.message?.content??'';for(let j=0;j<batch.length;j++){const m=c.match(new RegExp(j+1+'[.\\)]\\s*(MATCH|NO_MATCH)','i'));if(m){const key=batch[j]!.li+':'+batch[j]!.ri;(m[1].toUpperCase()==='MATCH'?llmMatch:llmDeny).add(key)}}}catch{}
  if((i/10)%20===0)process.stdout.write('.')
}
console.log('\nglm-4-0520: '+llmMatch.size+' match, '+llmDeny.size+' deny')
...
for(const thr of [0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85]){
  ...
  for(const p of pairs){const key=p.li+':'+p.ri;let lbl=p.score;if(llmMatch.has(key))lbl=Math.max(p.score,0.9);else if(llmDeny.has(key))lbl=Math.min(p.score,0.50);if(lbl>=thr)pred.add(abt[p.li]!.id+'|'+buy[p.ri]!.id)}
...
console.log('\n═══════════════════════════════════')
console.log('  v26: glm-4-0520 (few-shot, 800 reviews)')
console.log('═══════════════════════════════════')
...
console.log('═══════════════════════════════════')
console.log('v20 (DeepSeek-v4-flash 2400): 0.582')
console.log('v26 (glm-4-0520 800): '+bF1.toFixed(4))
}
main()
