...
for(let i=0;i<abt.length;i++){
  ...
  for(let j=0;j<buy.length;j++){
    ...
    if(sh>=2){const jw=jaroWinklerScorer.score(aN[i]!,bN[j]!,meta);if(jw>=0.35){const emb=cos(aEmb[i]!,bEmb[j]!);const s=jw*0.50+emb*0.50;if(s>=0.32)pairs.push({li:i,ri:j,score:s,jw,emb,nameA:aN[i]!,nameB:bN[j]!})}}
  }
}
pairs.sort((a,b)=>b.score-a.score)
console.log('\n'+pairs.length+' candidates in '+((performance.now()-t0)/1000).toFixed(1)+'s')

// v20: 2400 LLM + temperature=0.1 + graduated penalty
const boundary=pairs.filter(p=>p.score>=0.32&&p.score<=0.95).slice(0,2400)
console.log('LLM reviewing '+boundary.length+' pairs (T=0.1)')
...
for(let i=0;i<boundary.length;i+=10){
  ...
  const body=JSON.stringify({model:'deepseek-v4-pro',messages:[{role:'user',content:prompt}],max_tokens:400,temperature:0.1})
...
// v20: GRADUATED penalty — denied with high JW get less penalty
...
for(const thr of [0.35,0.4,0.45,0.5,0.52,0.55,0.58,0.6,0.65,0.7,0.75,0.8,0.85]){
  ...
  for(const p of pairs){
    ...
    else if(llmDeny.has(key)){
      if(p.jw>0.65) lbl=Math.min(p.score,0.65)      // strong jw→softer deny
      else if(p.jw>0.55) lbl=Math.min(p.score,0.55) // moderate jw→moderate deny
      else lbl=Math.min(p.score,0.40)               // weak jw→hard deny
    }
...
console.log('\n═══════════════════════════════════')
console.log('  v20: 2400 LLM + graduated penalty + T=0.1')
console.log('═══════════════════════════════════')
...
console.log('═══════════════════════════════════')
console.log('v19: 0.581 | v20: '+bF1.toFixed(4)+' | GM: 0.628')
}
main()
