...
const {extractElectronicsFields,scoreProductPair}=await import('../packages/entity-resolver-core/dist/domain/electronics.js')
...
// v21: jw + Zhipu embedding + EXACT domain matching
console.log('Scoring with jw(0.45)+emb(0.40)+domain(0.15) [EXACT model/brand]...')
const t0=performance.now()
const pairs:Array<{li:number,ri:number,score:number,jw:number,emb:number,domain:number,nameA:string,nameB:string}>=[]
for(let i=0;i<abt.length;i++){
  ...
  for(let j=0;j<buy.length;j++){
    ...
    if(sh>=2){
      const jw=jaroWinklerScorer.score(aN[i]!,bN[j]!,meta);if(jw>=0.35){
        const emb=cos(aEmb[i]!,bEmb[j]!)
        const dm=scoreProductPair(aF[i]!,bF[j]!)
        const s=jw*0.45+emb*0.40+dm.score*0.15
        if(s>=0.32)pairs.push({li:i,ri:j,score:s,jw,emb,domain:dm.score,nameA:aN[i]!,nameB:bN[j]!})
      }
    }
  }
}
...
// LLM 1600 reviews
const boundary=pairs.filter(p=>p.score>=0.35&&p.score<=0.90).slice(0,1600)
console.log('LLM reviewing '+boundary.length+' pairs')
...
for(const thr of [0.35,0.4,0.45,0.5,0.52,0.55,0.58,0.6,0.65,0.7,0.75,0.8,0.85]){
  const pred=new Set<string>()
  for(const p of pairs){
    const key=p.li+':'+p.ri;let lbl=p.score
    if(llmMatch.has(key))lbl=Math.max(p.score,0.9)
    else if(llmDeny.has(key))lbl=Math.min(p.score,0.50)
...
console.log('\n═══════════════════════════════════')
console.log('  v21: EXACT domain + Zhipu + LLM')
console.log('═══════════════════════════════════')
...
console.log('═══════════════════════════════════')
console.log('v20 (no domain): 0.582 | v21 (+EXACT domain): '+bF1.toFixed(4))
}
main()
