// Abt-Buy v8: dual LLM review (boundary + high-confidence), 600 pairs
...
async function main() {
  ...
  // v8: dual review strategy
  // Zone A: boundary [0.40, 0.75] → LLM finds missed matches (recall boost)
  // Zone B: high-conf [0.75, 0.95] → LLM catches false matches (precision boost)
  const zoneA = pairs.filter(p=>p.score>=0.40&&p.score<=0.75).slice(0,300);
  const zoneB = pairs.filter(p=>p.score>=0.75&&p.score<=0.95).slice(0,300);
  const toReview = [...zoneA, ...zoneB];
  console.log('Zone A (boundary): '+zoneA.length+' | Zone B (high-conf): '+zoneB.length);
  console.log('LLM reviewing '+toReview.length+' total pairs...');

  const llmMatch = new Set<string>(), llmDeny = new Set<string>();
  let calls = 0;

  for(let i=0;i<toReview.length;i+=10){
    const batch = toReview.slice(i,i+10);
  ...
  // v8 scoring: match→0.9, deny→0.4, unreviewed→keep
  ...
  console.log('\n═══════════════════════════════════');
  console.log('  Abt-Buy v8: Dual LLM Review');
  console.log('═══════════════════════════════════');
  ...
  console.log('═══════════════════════════════════');
  console.log('v7: 0.413 | GoldenMatch zero-shot: 0.628');
}
main();
