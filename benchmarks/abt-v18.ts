...
// Score with v16 best weights (JW_W=0.55) — Zhipu quality allows more embedding weight
...
for(let i=0;i<abt.length;i++){
  ...
  for(let j=0;j<buy.length;j++){
    ...
    if(sh>=2){const jw=jaroWinklerScorer.score(aN[i]!,bN[j]!,meta);if(jw>=0.35){const emb=cos(aEmb[i]!,bEmb[j]!);const s=jw*0.50+emb*0.50;if(s>=0.35)pairs.push({li:i,ri:j,score:s,jw,emb,nameA:aN[i]!,nameB:bN[j]!})}}
  }
}
pairs.sort((a,b)=>b.score-a.score)
console.log('\n'+pairs.length+' candidates in '+((performance.now()-t0)/1000).toFixed(1)+'s')

// v17: 1600 LLM reviews + softer deny penalty (0.48) + wider boundary [0.35,0.92]
const boundary=pairs.filter(p=>p.score>=0.35&&p.score<=0.92).slice(0,1600)
...
// v17: softer penalty + full pair scoring
...
console.log('\n═══════════════════════════════════')
console.log('  v17: 1600 LLM + softer penalty + 50/50 weights')
console.log('═══════════════════════════════════')
...
console.log('═══════════════════════════════════')
console.log('v16 (800 LLM): 0.522 | v17 (1600 LLM): '+bF1.toFixed(4)+' | GM: 0.628')
}
main()
