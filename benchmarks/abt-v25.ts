...
const boundary=pairs.filter(p=>p.score>=0.35&&p.score<=0.90).slice(0,800)
console.log('GLM-4-Plus reviewing '+boundary.length+' pairs')
const llmMatch=new Set<string>(),llmDeny=new Set<string>()
for(let i=0;i<boundary.length;i+=10){
  const batch=boundary.slice(i,i+10)
  let prompt='Same product? Output ONLY "N. MATCH" or "N. NO_MATCH" for each:\n\n'
  for(let j=0;j<batch.length;j++)prompt+=(j+1)+'. '+batch[j]!.nameA.slice(0,50)+'\n   '+batch[j]!.nameB.slice(0,50)+'\n\n'
  const body=JSON.stringify({model:'glm-4-plus',messages:[{role:'user',content:prompt}],max_tokens:200,temperature:0})
  const r=spawnSync('curl',['-s','-m','25','https://open.bigmodel.cn/api/paas/v4/chat/completions','-H','Content-Type: application/json','-H','Authorization: Bearer '+ZP_K,'-d',body],{encoding:'utf-8',timeout:30000})
...
console.log('\nGLM-4-Plus: '+llmMatch.size+' match, '+llmDeny.size+' deny')
...
console.log('\n═══════════════════════════════════')
console.log('  v25: GLM-4-Plus (800 reviews)')
console.log('═══════════════════════════════════')
...
console.log('v25 (GLM-4-Plus 800): '+bF1.toFixed(4))
}
main()
